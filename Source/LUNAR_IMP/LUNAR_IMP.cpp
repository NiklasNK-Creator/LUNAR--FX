	...
	PF_LayerDef		*output)
{
	...
	PF_ADD_POPUP("Impact Frame", 0, 2, "No\0Yes", IMPACT_FRAME_DISK_ID);
	
	AEFX_CLR_STRUCT(def);
	PF_ADD_POPUP("Hold", 0, 2, "No\0Yes", HOLD_DISK_ID);
	...
	AEFX_CLR_STRUCT(def);
	PF_ADD_POPUP("Edge Type", 0, 7, "Fade\0Hard\0Flame\0Paint\0Grade\0Blend\0Rough", EDGE_TYPE_DISK_ID);
